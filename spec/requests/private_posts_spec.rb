...
RSpec::describe "Posts with authentication", type: :request do
  ...
  let!(:other_user_draft_post) { create(:post, user_id: other_user.id, published: false) }
    ...
    context "with valid auth" do
      context "when requisting other's author post" do
        ...
        context "when post is a draft" do
          before { get "/posts/#{other_user_draft_post.id}", headers: auth_headers }
  ...
  private
    def payload
      # esto es para obtener un hash que se pueda 
      # acceder con "id" o con :id
